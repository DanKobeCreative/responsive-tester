...
- **Existing iframe preview grid still loads URLs correctly under the new lib.rs dev-mode branch.** First launch after the lib.rs change was blank because of the localhost-plugin issue; subsequent launches load, but I have not driven the Preview side end-to-end (loading a URL, seeing all 20 frames render, scaling, screenshotting, inspecting). Verifying with the `cargo tauri dev` Dan is currently running.
...
- **Cancel mid-run.** No zombie-Node check (`pgrep node` clean after cancellation) has been run live.
