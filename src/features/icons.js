// Lucide-style SVG icons. 24x24 viewbox, 2px stroke, round caps — matches
// the Lucide spec so icons feel visually consistent across the app.
// Hand-picked paths to avoid a runtime dependency.
...
const ICONS = {
  ...
  'rows':
    '<rect width="18" height="18" x="3" y="3" rx="2"/><path d="M21 12H3"/>',
  'columns':
...
export function icon(name, size = 14) {
  ...
  if (!paths) return '';
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="rt-icon" aria-hidden="true">${paths}</svg>`;
}
